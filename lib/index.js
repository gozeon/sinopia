...
module.exports = function(config_hash) {
  ...
  var morganFile = config.morgan || path.join(__dirname, 'access.log');
 
  morgan.token('response-time-seconds', function getResponseTimeInSeconds(req, res) {
    return this['response-time'](req, res) / 1000
  })

  app.use(morgan(':remote-addr - :remote-user [:date[clf]] ":method :url HTTP/:http-version" :status :res[content-length] ":referrer" ":user-agent" :response-time-seconds', {
    stream: fs.createWriteStream(morganFile, { flags: 'a' })
  }))
