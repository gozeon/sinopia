...
var jsonMorgan = require('morgan-json');
...
module.exports = function(config_hash) {
  ...
  mongoMorgan.token('url-format', function getUrlToken(req, res) {
    return this['url'](req, res).replace(/\%2F/gi, '/').substr(1)
  })

  var morganFormat = ':remote-addr :method :url-format :date[iso] :status :res[content-length] :user-agent :response-time'


  app.use(mongoMorgan('mongodb://localhost:27017', jsonMorgan(morganFormat), {
    collection: 'logs'
  }))
